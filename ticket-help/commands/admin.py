...
@app_commands.command(
    name="remove-claimer",
    description="Remove a helper from a ticket"
)
async def remove_claimer(
    ...
    if user.id not in claimers:
        return await interaction.response.send_message(
            f"ℹ️ {user.mention} is not a claimer on this ticket.",
            ephemeral=True
        )
    ...
    await interaction.response.send_message(
        f"🧹 Removed {user.mention} from ticket **{ticket_name}**.",
        ephemeral=True
    )
...
async def add_points(
    ...
    if not user_data:
        return await interaction.response.send_message(
            f"❌ User `{user.name}` does not exist.",
            ephemeral=True
        )
    ...
    await interaction.response.send_message(
        f"✅ Points for user `{user.name}` updated.",
        ephemeral=True
    )
...
@app_commands.command(
    name="set-boss-points",
    description="Set the points for a boss"
)
async def set_boss_points(
    ...
    if not doc.exists:
        return await interaction.response.send_message(
            f"❌ Boss `{boss_name}` does not exist.",
            ephemeral=True
        )
    ...
    await interaction.response.send_message(
        f"✅ Points for boss `{boss_name}` set to `{points}`.",
        ephemeral=True
    )
...
@app_commands.command(
    name="toggle-boss-available",
    description="Toggle boss availability"
)
async def toggle_boss_available(
    ...
    if not doc.exists:
        return await interaction.response.send_message(
            f"❌ Ticket type `{type_name}` does not exist.",
            ephemeral=True
        )
...
@app_commands.command(
    name="list-bosses",
    description="List all bosses for type"
)
async def list_bosses(
    ...
    bosses = doc.to_dict().get("bosses", [])
    await interaction.response.send_message(
        f"Bosses for type **{type_name}**: {', '.join(bosses)}",
        ephemeral=True
    )
...
async def add_type(
    ...
    if doc.exists:
        return await interaction.response.send_message(
            f"❌ Ticket type `{type_name}` already exists.",
            ephemeral=True
        )
    ...
    await interaction.response.send_message(
        f"✅ Ticket type **{type_name}** has been created.",
        ephemeral=True
    )
...
async def delete_type(
    ...
    await interaction.response.send_message(
        f"🗑 Ticket type **{type_name}** has been deleted.",
        ephemeral=True
    )
...
async def delete_boss(interaction: discord.Interaction, ticket_type: str, boss: str):
    ...
    if doc.exists:
        doc_ref.update({
            "bosses": firestore.ArrayRemove([boss])
        })
    else:
        return await interaction.response.send_message(
            f"❌ **{ticket_type}** does not exist.",
            ephemeral=True
        )

    await interaction.response.send_message(
        f"✅ Removed **{boss}** from **{ticket_type}** bosses.",
        ephemeral=True
    )
...
@app_commands.command(
    name="add-boss",
    description="Add a boss to a ticket type with points"
)
async def add_boss(
    interaction: discord.Interaction,
    ticket_type: str,
    ...
    if type_doc.exists:
        type_ref.update({
            "bosses": firestore.ArrayUnion([boss])
        })
    else:
        type_ref.set({
            "bosses": [boss]
    ...
    await interaction.response.send_message(
        f"✅ Added **{boss}** to **{ticket_type}** with **{points}** points.",
        ephemeral=True
    )

@app_commands.command(
    name="reset-all-points",
    description="Archive and reset all user points"
)
...
@app_commands.command(
    ...
    description="Manually set a user's points"
)
async def set_user_points(
    ...
    await interaction.response.send_message(
        f"✅ Set **{user.mention}** points to **{points}**.",
        ephemeral=True
    )
...
@app_commands.command(
    name="reload-point-rules",
    description="Reload ticket point rules from the database"
)
async def reload_point_rules(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        "🔄 Point rules cache reloaded from Firestore.",
        ephemeral=True
    )

@app_commands.command(
    name="update-points",
    description="Set how many points a boss-related ticket gives"
)
async def set_boss_points(
    interaction: discord.Interaction,
    boss: str,
    ...
    await interaction.response.send_message(
        f"✅ Boss **{boss}** now awards **{points} points** per ticket.",
        ephemeral=True
    )
